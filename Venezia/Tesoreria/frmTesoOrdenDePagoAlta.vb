...
Public Class frmTesoOrdenDePagoAlta
    Public gOrdenDePago As cOrdenDePago = Nothing
    ...
    Private Sub subCargarDatos()
        Try
            dtpFecha.Value = gOrdenDePago.Fecha
            txtTotalEfectivo.Text = gOrdenDePago.Importe_cash
            txtTotalTransf.Text = gOrdenDePago.Importe_transferencia
            lblEstado.Text = gOrdenDePago.Estado.Estado

            If gOrdenDePago.Importe_cheques > 0 Then
                AgregarCheques(gOrdenDePago.Cheques)
            End If
    ...
    Private Sub btnGuardar_Click(sender As Object, e As EventArgs) Handles btnGuardar.Click
        ...
        Try
            ...
            If Me.TipoDeOperacion = EnuOPERACION.ALTA Or Me.TipoDeOperacion = EnuOPERACION.MODIF Then
                If Me.TipoDeOperacion = EnuOPERACION.ALTA Then
                    gOrdenDePago = New cOrdenDePago(gAdmin)
                    gOrdenDePago.EsNuevo = True
                End If

                'GUARDO LA INFORMACION
                gOrdenDePago.Importe_cash = txtTotalEfectivo.Text.Trim
                gOrdenDePago.Importe_cheques = subActualizarSumCheques()
                gOrdenDePago.Importe_transferencia = txtTotalTransf.Text.Trim
                gOrdenDePago.Fecha = dtpFecha.Value
                gOrdenDePago.Observaciones = txtObservac.Text.Trim
                gOrdenDePago.Tipo_Destino = cOrdenDePago.enuTipoDestinoOrdenPagoGetEnuxStr(cmbDestino.Text)
                If gOrdenDePago.Tipo_Destino = cOrdenDePago.enuTipoDestinoOrdenPago.Proveedores Then
                    gOrdenDePago.Proveedor = DirectCast(txtProove.Tag, cProveedor)
                End If
                If gOrdenDePago.Importe_cheques > 0 Then
                    gOrdenDePago.Cheques = New ArrayList
                    For Each lItem In lvwConsulta.Items
                        gOrdenDePago.Cheques.Add(lItem.Tag)
                    Next
                End If

                If gOrdenDePago.Guardar() = False Then
